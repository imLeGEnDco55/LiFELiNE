...
export function TasksPage() {
  ...
  // Unified memoization for O(N) single-pass processing
  const { groupedPending, pendingCount, completedList } = useMemo(() => {
    ...
    subtasks.forEach(subtask => {
      ...
      const subtaskWithDeadline = { ...subtask, deadline };

      if (subtask.completed) {
        completed.push(subtaskWithDeadline);
      } else {
        if (!grouped[subtask.deadline_id]) {
          grouped[subtask.deadline_id] = [];
        }
        grouped[subtask.deadline_id].push(subtaskWithDeadline);
        pCount++;
      }
    });
  ...
  return (
    <div className="px-4 py-6">
      ...
      <motion.div
        ...
        className="space-y-4"
      >
        {Object.entries(groupedPending).map(([deadlineId, pendingTasks]) => {
